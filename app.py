...
def run(cmd: list[str], cwd: Path | None = None) -> str:
    ...
    if proc.returncode:
        ...
        if not detail and proc.returncode < 0:
            detail = f"{cmd[0]} was stopped by signal {-proc.returncode}; the server ran out of available resources"
        raise RuntimeError(detail or f"{cmd[0]} failed with exit code {proc.returncode}")
    return proc.stdout
...
Style: Pop,DejaVu Sans,56,&H00FFFFFF,&H0000FFFF,&H00101010,&H70000000,-1,0,0,0,100,100,0,0,1,5,2,2,55,55,215,1
...
def render_clip(video: Path, words: list[Word], segment: Segment, output: Path, index: int) -> None:
    ...
    run([
        ...
        "-vf", filters, "-c:v", "libx264", "-preset", "ultrafast", "-threads", "1", "-crf", "21",
